...
module.exports = {
	...
	modifyScope(scope, mutator, filter, discriminator) {
		const ScopeBodyMap = {
			'Program': ['body'],
			'BlockStatement': ['body'],
		...
		for (let body of ScopeBodyMap[scope.type].map(key => scope[key])) {
			let finalised = new Set();
			while (true) {
				let index = body.findIndex(node => filter(node) && !finalised.has(discriminator(node)));
				if (index === -1) {
					break;
				}
				let node = body[index];
