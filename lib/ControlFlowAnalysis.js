...
function analyseDominators(nodeMap, entryNode) {
	for (let node in nodeMap) {
		nodeMap[node].dominators = new Set((node - 0 === entryNode) ?
			[entryNode] :
	...
	do {
		...
		for (let node in nodeMap) {
			...
			for (let predecessor of nodeMap[node].predecessors) {
				let oldDominators = new Set(nodeMap[node].dominators);
				let newDominators = nodeMap[node].dominators = new Set([...oldDominators].filter(
					d => d === node || nodeMap[predecessor].dominators.has(d)));
...
export function recoverStatements(nodeMap, initLabel, exitNode) {
	analysePredecessors(nodeMap, initLabel, exitNode);
	analyseDominators(nodeMap, initLabel);
	let loops = {};
	...
	while (current !== exitNode) {
		if (current in loops) {
			...
			if (leader.test) {
				...
				throw `Informal loop ${initLabel} -> ${loop} -> ${exitNode} (conditional not at front or back)`;
			}
			if (loop.indexOf(testblock.consequent) === -1) {
				...
				throw `Informal loop ${initLabel} -> ${loop} -> ${exitNode} (FIXME: infinite loop)`;
			}
			for (let block of blocks) {
				if (nodeMap[block].test && testblock !== trailer) {
					throw `Informal loop ${initLabel} -> ${loop} -> ${exitNode} (conditional within loop)`;
				}
