...
function reduceSequence(controlFlow) {
	...
	for (let [parentLabel, parentNode] of Object.entries(nodeMap)) {
		...
		if (childNode.predecessors.size !== 1) {
			continue;
		}
...
function reduceSimpleWhile(controlFlow) {
	...
	for (let [parentLabel, parentNode] of Object.entries(nodeMap)) {
		...
		if (parentNode.consequent === parentNode.alternate) { //??
			continue;
		}

		let consequentNode = nodeMap[parentNode.consequent];
		let alternateNode = nodeMap[parentNode.alternate];
	
		// single block while
		if (parentNode.consequent === parentLabel) {
			parentNode.predecessors.delete(parentLabel);
			parentNode.statementsBefore.push({
				type: 'WhileStatement',
				test: parentNode.test,
				body: {
					type: 'BlockStatement',
					body: parentNode.statementsAfter.concat(parentNode.statementsBefore),
				},
			});
			parentNode.consequent = parentNode.alternate;
			parentNode.alternate = parentNode.test = null;
			alternateNode.statementsBefore = parentNode.statementsAfter.concat(alternateNode.statementsBefore);

			parentNode.statementsAfter = [];
			reduced = true;
			continue;
		} else if (parentNode.alternate === parentLabel) {
			parentNode.predecessors.delete(parentLabel);
			parentNode.statementsBefore.push({
				type: 'WhileStatement',
				test: {
			...
			parentNode.alternate = parentNode.test = null;
			consequentNode.statementsBefore = parentNode.statementsAfter.concat(consequentNode.statementsBefore);
		...
		// two block while
		if (parentNode.consequent !== null &&
			consequentNode.consequent === parentLabel &&
			...
			parentNode.statementsBefore.push({
				...
				body: {
					type: 'BlockStatement',
					body: parentNode.statementsAfter.concat(
						consequentNode.statementsBefore,
						consequentNode.statementsAfter,
						parentNode.statementsBefore),
				},
			});
			
			parentNode.predecessors.delete(parentNode.consequent);
			alternateNode.statementsBefore = parentNode.statementsAfter.concat(alternateNode.statementsBefore);
			parentNode.statementsAfter = [];
			...
			consequentNode.__deleted = true;
			reduced = true;
		} else if (alternateNode.consequent === parentLabel &&
			...
			parentNode.statementsBefore = parentNode.statementsBefore.concat([{
				...
				body: {
					type: 'BlockStatement',
					body: parentNode.statementsAfter.concat(
						alternateNode.statementsBefore,
						alternateNode.statementsAfter,
						parentNode.statementsBefore),
				},
			}]);
			
			parentNode.predecessors.delete(parentNode.alternate);
			consequentNode.statementsBefore = parentNode.statementsAfter.concat(consequentNode.statementsBefore);
			parentNode.statementsAfter = [];

			parentNode.alternate = parentNode.test = null;

			alternateNode.__deleted = true;
			reduced = true;
		}
	}
...
export function recoverStatements(controlFlow) {
	...
	if (Object.keys(controlFlow.nodes).length === 1) {
		...
		return node.statementsBefore.concat(node.statementsAfter);
	} else {
	}
}
