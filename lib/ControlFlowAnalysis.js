	...
	while (current !== exitNode) {
		if (current in loops) {
			...
			if (leader.test) {
				...
				throw Error(`Informal loop ${entryNode} -> ${loop} -> ${exitNode} (conditional not at front or back)`);
			}
			if (loop.indexOf(testblock.consequent) === -1) {
				...
				throw Error(`Informal loop ${entryNode} -> ${loop} -> ${exitNode} (FIXME: infinite loop)`);
			}
			for (let block of blocks) {
				if (nodeMap[block].test && testblock !== trailer) {
					throw Error(`Informal loop ${entryNode} -> ${loop} -> ${exitNode} (conditional within loop)`);
				}
			...
			if (consequentNodes[consequentNodes.length - 1] !== alternateNodes[alternateNodes.length - 1]) {
				throw Error(`Nested if in ${current}`);
			}
