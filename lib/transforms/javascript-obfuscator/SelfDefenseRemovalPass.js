...
const selfDefenseSignatures = [
	utils.parseExpression('$_Identifier_.constructor(\'return /" + this + "/\')().constructor($_IGNORE_)'),
	utils.parseExpression('$_Identifier$1_.toString().search($_Literal_).toString().constructor($_Identifier$1_).search($_Literal_);'),
];
...
function isSelfDefense(decl) {
	...
	estraverse.traverse(innerFunc, {
		enter: (statement) => {
			for (let signature of selfDefenseSignatures) {
				if (utils.specMatch(statement, signature)) {
					hasRegex = true;
