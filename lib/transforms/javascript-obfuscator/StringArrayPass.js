...
function resolveWrappers(tree, decoders) {
	estraverse.traverse(tree, {
		enter: (scope) => utils.modifyScope(
			scope,
			(node, index, body) => {
				let names = Object.fromEntries(node.declarations.filter(decl => decl?.init.type == 'Identifier' && decl.init.name in decoders).map(decl => [decl.id.name, decl.init.name]));
				node.declarations = node.declarations.filter(decl => !(decl.id.name in names));
			...
			(node) => node.type == 'VariableDeclaration' &&
				node.declarations.some(decl => decl.init?.type == 'Identifier' && decl.init.name in decoders),
