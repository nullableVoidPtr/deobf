				...
				if (utils.specMatch(node, utils.parseExpression(`$_FunctionExpression_(${utils.escapeIdentifier(arrayIdent)}, $_Literal_)`))) {
					...
					let isRotatePredicate = (expression) => {
						...
						if (utils.specMatch(expression, utils.parseExpression('parseInt($_Identifier_($_IGNORE_))'))) {
							encodedNumbers.push({
								decoder: expression.arguments[0].callee.name,
								args: expression.arguments[0].arguments.map(v => v.value)
							});
							return true;
						}
					...
					if (!isIterative) {
						...
						for (let offset = 0; offset < array.length; offset++) {
							if (encodedNumbers.every(({decoder, args}) => {
								try {
									if (decoder in decoders && !isNaN(parseInt(decoders[decoder](...args)))) {
										return true;
									}
								} catch {}
							})) {
								let cloned = JSON.parse(JSON.stringify(expression));
								estraverse.replace(cloned, {
									enter(int) {
										if (utils.specMatch(int, utils.parseExpression('parseInt($_Identifier_($_IGNORE_))')) &&
											int.arguments[0].callee.name in decoders) {
											return {
												type: 'Literal',
												value: parseInt(decoders[int.arguments[0].callee.name](...int.arguments[0].arguments.map(v => v.value)))
											};
										}
									}
								});
								if (node.arguments[1].value == eval(escodegen.generate(cloned)))
									break;
							}
...
export default (tree, options) => {
	...
	tree = estraverse.replace(tree, {
		enter: (node) => {
			if (decoders !== null) {
				if (utils.specMatch(node, utils.parseExpression("$_Identifier_()")) &&
					Object.keys(decoders).includes(node.callee.name)) {
					let args = node.arguments.map((literal) => literal.value);
					return {
						type: 'Literal',
						value: decoders[node.callee.name](...args),
					};
			...
			if (!stringArrayRemoved &&
				utils.specMatch(node, utils.parseStatement(`var ${utils.escapeIdentifier(arrIdent)} = []`))) {
