...
function findControlFlowStorage(statements) {
	for (var statement of statements) {
		if (statement.type === 'VariableDeclaration') {
			for (var objectDecl of statement.declarations) {
...
function tagScope(scope) {
	...
	if (scope.type === 'BlockStatement') {
		...
		for (var switchCase of scope.cases) {
...
module.exports = class ControlFlowStoragePass extends BasePass {
	...
	static _transform(tree) {
		...
		estraverse.replace(tree, {
			enter: (scope) => {
				if (typeof scope.controlFlowStorage === 'undefined') {
					return;
				}
				scope = estraverse.replace(scope, {
					enter: (node) => {
						if (node.type === 'VariableDeclarator' &&
							node.id.name === scope.controlFlowStorage.name) {
							replaced = true;
						...
						if (node.type === 'CallExpression' &&
							...
							node.callee.object.name === scope.controlFlowStorage.name &&
							node.callee.property.type === 'Literal') {
							let funcExpression = scope.controlFlowStorage.object[node.callee.property.value];
						...
						if (node.type === 'MemberExpression' &&
							...
							node.object.name === scope.controlFlowStorage.name &&
							node.property.type === 'Literal') {
							replaced = true;
							return scope.controlFlowStorage.object[node.property.value];
						}
					}
				});
				delete scope.controlFlowStorage;
				return scope;
			}
		});
